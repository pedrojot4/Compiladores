...
{
    // Função auxiliar para converter inteiros em booleanos
    auto convertIntToBool = [](Expression* expr) {
        if (expr->Type() == "int") {
            int value = stoi(expr->token->lexeme);  // Converte o lexema do token para int
            expr->type = ExprType::BOOL;  // Atualiza o tipo para booleano
            expr->token->tag = (value == 0) ? Tag::FALSE : Tag::TRUE;  // Define a tag como TRUE ou FALSE
        }
    };

    // Converte expr1 e expr2 para booleanos, se forem inteiros
    convertIntToBool(expr1);
    convertIntToBool(expr2);

    // Valida se ambos os operandos são do tipo booleano
    if (expr1->type != ExprType::BOOL || expr2->type != ExprType::BOOL) {
        // Cria uma mensagem de erro detalhada para o operador e tipos dos operandos
        stringstream ss;
        ss << "\'" << token->lexeme << "\' usado com operandos não booleanos ("
           << expr1->Name() << ":" << expr1->Type() << ") ("
           << expr2->Name() << ":" << expr2->Type() << ") ";
        // Lança um erro de sintaxe com o número da linha e a mensagem
