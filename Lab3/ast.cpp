...
void While::Gen() {
    ...
    // Avalia a expressão condicional e converte para string uma única vez
    Expression *condition = Rvalue(expr);
    std::string conditionStr = condition->ToString();

    // Se a condição for falsa, pula para o final do loop (Lafter)
    cout << "\tifFalse " << conditionStr << " goto L" << after << endl;

    // Gera o código para o corpo do loop
    stmt->Gen();

    // Retorna ao início do loop para reavaliar a condição
